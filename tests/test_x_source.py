...
from net_razor.sources.x.bird_backend import XSearchTuning
...
def _settings(**overrides) -> Settings:
    values = {"auth_token": "tok", "ct0": "ct"}
    values.update(overrides)
    return Settings(_env_file=None, **values)


# No pacing delay in tests -- the real one is a deliberate 1s gap between requests.
_NO_WAIT = XSearchTuning(request_spacing_seconds=0, retry_backoff_seconds=0)
...
async def test_x_source_normalizes_and_records_raw():
    ...
    source = XSource(_settings(), backend, tuning=_NO_WAIT)
    ...
    # auth_status is a result, so it rides in meta -- effective_request records
    # only what was asked for, and must not vary with prior calls.
    assert result.meta["auth_status"] == "valid"
    assert "auth_status" not in result.effective_request
